...
; Problem 1
; Replace boxes in set-box! instead of just adding a new one
; Define a function remove-element which deletes cells at a specific location
(define (remove-element [lst : (listof Storage)] [location : Location]) : (listof Storage)
  (type-case Storage (first lst)
    [cell (l v) (cond
                  [(equal? l location) (rest lst)]
                  [else (cons (cell l v) (remove-element (rest lst) location))])]))

(test (remove-element (cons (cell 1 (numV 1)) (cons (cell 2 (numV 2)) empty)) 1) (cons (cell 2 (numV 2)) empty))
